...
class LinearGradientControl(ColorControl):
    """
    ...
    def __init__(self):
        super().__init__(name="LinearGradient", actual_range=(0, 1), ui_range=(0, 1))
        self.decimals = 3
    ...
    def create_widgets(self, parent: QWidget):
        ...
        self.saturation_pair.slider.setValue(LinearGradientControl.saturation)
        ...
        # Initialize from the shared class variable.
        self.rotation_pair.slider.setValue(LinearGradientControl.hue)
        ...
        # Initialize from the shared class variable.
        self.spinbox.setValue(LinearGradientControl.num_colors)
    ...
    def on_saturation_changed(self, value):
        LinearGradientControl.saturation = value
        self.compute_arc()

    def on_hue_changed(self, value):
        LinearGradientControl.hue = value
        self.rotate_arc()

    def on_num_colors_changed(self, value):
        LinearGradientControl.num_colors = value
        self.draw_buttons()
    ...
    def compute_arc(self):
        ...
        if len(colors) != 2:
            self.color_arc = ColorArcSingular(
                colors[0],
                num_colors,
                sat_val
            )
        else:
            self.color_arc = ColorArc(
                colors[0],
                colors[1],
                num_colors,
                sat_val
...
class ColorTetraControl(ColorControl):
    def __init__(self):
        super().__init__(name="ColorTetra", actual_range=(0, 1), ui_range=(0, 1))
        self.decimals = 3
        self.current_colors = [QColorEnhanced()]  # Initialize with a default color
        ...
        self.color_tetra = None  # Will hold a ColorTetra or TwoColorTetra instance
    ...
    def compute_tetra(self):
        ...
        if len(self.current_colors) == 2:
            self.color_tetra = TwoColorTetra(
            ...
            # If we don't have exactly 2 colors, use the first color (or a default if empty)
            color = self.current_colors[0] if self.current_colors else QColorEnhanced()
            self.color_tetra = ColorTetra(
                color,
                sat_val,
                rotation_val
            )
    ...
    def update_widgets(self, colors):
        if not colors:
            return
        # Always treat input as a list of colors
        self.current_colors = colors if isinstance(colors, list) else [colors]
        
        # Update slider colors using first color
        if len(self.current_colors) > 0:
            self.saturation_pair.set_handle_color(self.current_colors[0])
    ...
    def swatch_clicked(self, index):
        """
        Copy color to clipboard and notify the parent of the color change
        """
        color = self.current_gradient_colors[index]
